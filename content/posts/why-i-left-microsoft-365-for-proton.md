...
A OneDrive replacement was essential, and apps for Drive, Mail and Calendar had to be native across all platforms I use. The one gap here is Proton Drive on Linux — there's no native desktop client yet, though one is on the way. The web version is good enough in the meantime.

### Never using the same email twice
I never use the same email address twice, so access to catch-all configuration and unlimited aliases (via SimpleLogin, which is bundled with Proton Unlimited) was also very important.

### Continuity 
The migration process must have zero downtime, with no emails lost.

### Cost
Costs had to be reasonable, and I would happily have paid more than I was paying for Microsoft (Business Standard license) for the right product. 
